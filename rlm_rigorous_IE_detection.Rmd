...
Negative (positive) delta -> editing/depletion (enrichment) of
neo-antigens
...
reg_method = 'fit_dropout_aware_nb'
...
f_setting_dtf <-
  ...
  filter_coef_overview(
    ...
    # norm_scale_filter = .5,
    # scale_avg_norm_filter = .5,
    # adaptive_NMADR_q75_filter = T,
    # adaptive_delta_SE_filter = T,
    # delta_SE_filter = .2
  )
...
p <- ggplot(f_setting_dtf,
  aes_string(x = varname, y = 'norm_scale')) +
  geom_hex() +
...
print_plot(
  p, fn = file.path(img_loc, glue('norm_scale_vs_{varname}.png')),
  w = 17.4, h = 17)
```
...
print_plot(
  p1, fn = file.path(img_loc,
    glue('rlm_fishtail{make_flag(B)}\\
      {make_flag(varname)}{make_flag(response_var)}.png')),
  w = 17.4, h = 17)
```
...
varname = 'NMAD_prob_5'
varname = 'delta_SE'
response_var = 'intercept'
response_var = 'log10_n_patients'

p1 <- var_vs_error(
  dtf = f_setting_dtf[analysis_name != 'rooney_param_titration'],
  x = varname, y = response_var) + right_legend
...
source(file.path('~/antigenic_space', 'maarten-analyses',
    'immune_editing', 'continuous_IE_detection_init.R'))
# f_setting_dtf[, summary(delta_SE)]

plots <-
  expand_grid(
    delta_se = seq(0.0, 0.7, .1), delta = c(.2, 0, -.2), rep = 1:3
  ) %>%
  purrr::pmap(function(delta_se, delta, rep) {
    pick <- NULL; error <- 1e-9
    # mad = .4; delta = 0;
    while (is.null(pick$percentile_rank) || nrow(pick) == 0) {
      pick <- f_setting_dtf[order(
        (yr_fractional_change - delta)^2 + (delta_SE - delta_se)^2)]
      pick <- pick[rep]
      error <- error * 10
    }
    rlm_plot_pick(pick,
...
plot_panel_layout(plots,
  filename = file.path(img_loc,
    'delta_se_titration_scatters.pdf'),
...
source(file.path('~/antigenic_space', 'maarten-analyses',
    'immune_editing', 'continuous_IE_detection_init.R'))
# f_setting_dtf[, summary(NMADR_q75)]

plots <-
  expand_grid(
    mad = seq(0.1, 0.5, .1), delta = c(.2, 0, -.2), rep = 1:3
  ) %>%
  purrr::pmap(function(mad, delta, rep) {
    pick <- NULL; error <- 1e-9
    # mad = .4; delta = 0;
    while (is.null(pick$percentile_rank) || nrow(pick) == 0) {
      if (F) {
        ...
        pick <- f_setting_dtf[which.min(
          (yr_fractional_change - delta)^2 +
            (NMADR_q75 - mad)^2)]
        pick <- f_setting_dtf[order(
          (yr_fractional_change - delta)^2 + (NMADR_q75 - mad)^2)]
        pick <- pick[rep]
      }
      error <- error * 10
    }
